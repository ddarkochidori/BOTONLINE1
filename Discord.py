...
# AHORA LEE DESDE LA VARIABLE DE ENTORNO LLAMADA 'DISCORD_TOKEN'
TOKEN = os.getenv('DISCORD_TOKEN') 
PREFIX = '!!' 

# IDs de ROLES DE STAFF (¡Debes reemplazar estos números!)
# Haz clic derecho en el rol > Copiar ID
STAFF_ROLE_ID = 1413900760156213342  # Rol general de STAFF/Moderador
MOD_ROLE_ID = 1413900760156213343    # Rol de Mods o Administradores
...
# (El resto del código del bot sigue exactamente igual...)
...
async def apply_staff(ctx):
    ...
    try:
        ...
        if not staff_roles_to_mention:
             # Si no se encuentra ningun rol por ID, intenta buscar uno de administrador para dar permisos
             admin_role = discord.utils.get(ctx.guild.roles, permissions__administrator=True)
             if admin_role:
                 permission_role_ids.append(admin_role.id)
                 staff_roles_to_mention.append(admin_role.mention)
             else:
                 return await ctx.send("❌ Error: No se pudo identificar ningún rol de Staff con las IDs proporcionadas.")
        ...
        # ESTO CORRIGE EL ERROR 400 BAD REQUEST
...
async def giveaway(ctx, time: str, *, prize: str):
    ...
    end_time = datetime.datetime.now() + discord.timedelta(seconds=seconds)
    ...
    if reaction:
        # Recuperar solo a los usuarios que no son el bot
...
if __name__ == '__main__':
    # Usamos el token de la variable de entorno
    if TOKEN:
        bot.run(TOKEN)
    else:
        print("❌ ERROR: La variable de entorno 'DISCORD_TOKEN' no fue encontrada. Asegúrate de configurarla en Railway.")
